...
import { Shape, vec2, Scene, Sprite, Slides, Link } from "./engine.js";
window.addEventListener('load', () => {
    ...
    const moveFocusUp = () => {
        if (!document.pointerLockElement) {
            ...
            if (activeButton == buttons[0] || !buttons.includes(activeButton))
                ...
                activeButton.previousElementSibling.focus();
        }
    };
    /**
    ...
    const moveFocusDown = () => {
        if (!document.pointerLockElement) {
            ...
            if (activeButton == buttons.at(-1) || !buttons.includes(activeButton))
                ...
                activeButton.nextElementSibling.focus();
        }
    };
    //* Variables & Constants 👓
    let pause = true;
    ...
    const shootSound = new globalThis.Howl({
    ...
    controls.mouseMove(e => {
        if (document.pointerLockElement) {
            ...
            if (Math.abs(mousePos.x) > halfScreen.x)
                mousePos.x = Math.sign(mousePos.x) * halfScreen.x;
            if (Math.abs(mousePos.y) > halfScreen.y)
                mousePos.y = Math.sign(mousePos.y) * halfScreen.y;
