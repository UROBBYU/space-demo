...
window.addEventListener('load', () => {
    ...
    const draw = (time: number) => {
        ...
        if (controls.key.KeyW) ship.accelerate(0, speed)
        if (controls.key.KeyS) ship.accelerate(0, -speed)
        if (controls.key.KeyA) ship.accelerate(-speed, 0)
        if (controls.key.KeyD) ship.accelerate(speed, 0)
