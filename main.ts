...
class Shape extends Entity {
    ...
    constructor(x = 0, y = 0, verts: Vector2[]) {
        super(x, y)
...
window.addEventListener('load', () => {
    ...
    const round = (num: number, dig: number) => Math.floor(num * Math.pow(10, dig)) / Math.pow(10, dig)
    ...
    window.addEventListener('keydown', e => {
        ...
        else if (e.key == 's') keyS = true
        else if (e.key == 'a') keyA = true
        else if (e.key == 'd') keyD = true
    })

    window.addEventListener('keyup', e => {
        if (e.key == 'w') keyW = false
        else if (e.key == 's') keyS = false
        else if (e.key == 'a') keyA = false
        else if (e.key == 'd') keyD = false
    })
    ...
    const draw = (time: number) => {
        ...
        if (keyW) ship.accelerate(0, speed)
        if (keyS) ship.accelerate(0, -speed)
        if (keyA) ship.accelerate(-speed, 0)
        if (keyD) ship.accelerate(speed, 0)
